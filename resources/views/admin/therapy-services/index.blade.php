...
<head>
    ...
    <title>Therapy Service Catalog | KinderCare</title>
            ...
            <div class="app-content-header pt-2 pb-0">
                <div class="container-fluid">
                    <div class="page-banner d-flex justify-content-between align-items-center flex-wrap gap-2">
                        <div>
                            <h2><i class="bi bi-clipboard2-pulse me-2"></i>Therapy Service Catalog</h2>
                            <p>Manage therapy service types, durations, and billing rates</p>
                        </div>
                        <a href="{{ route('admin.therapy-services.create') }}" class="btn btn-light btn-sm fw-bold shadow-sm px-3" style="position:relative;z-index:1;">
                ...
                <div class="container-fluid">
                    ...
                    <div class="card shadow-sm border-0 mb-3">
                        <div class="card-body py-2">
                            <form method="GET" action="{{ route('admin.therapy-services.index') }}" class="row g-2 align-items-center">
                                <div class="col-md-4">
                                    <div class="input-group input-group-sm">
                                        <span class="input-group-text bg-light border-end-0"><i class="bi bi-search text-muted"></i></span>
                                        <input type="text" name="search" class="form-control form-control-sm border-start-0" placeholder="Search services..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-md-2">
                                ...
                                <div class="col-md-4 d-flex gap-2">
                                    ...
                                    <a href="{{ route('admin.therapy-services.index') }}" class="btn btn-outline-secondary btn-sm">Reset</a>
                                </div>
                            </form>
                        </div>
                    </div>
                                    ...
                                    <tbody>
                                        @forelse ($services as $service)
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge therapy-badge-{{ $service->therapy_type }} border" style="font-size:0.75rem;">
                                                        {{ strtoupper($service->therapy_type) }}
                                                    </span>
                                                </td>
                                                <td><span class="text-dark">{{ $service->duration_minutes }} min</span></td>
                                                <td><span class="fw-semibold text-dark">৳{{ number_format($service->session_rate, 2) }}</span></td>
                                                <td>
                                                ...
                                                <td class="text-end">
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="{{ route('admin.therapy-services.edit', $service) }}" class="btn btn-sm btn-outline-primary" title="Edit">
                                                            <i class="bi bi-pencil"></i>
                                                        </a>
                                                        <form action="{{ route('admin.therapy-services.destroy', $service) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this therapy service?');">
                                                            @csrf @method('DELETE')
