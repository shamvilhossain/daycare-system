...
<head>
    ...
    <title>Create Program | Daycare System</title>
    <!-- Google Font -->
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
    <div class="app-wrapper">
        ...
        {{-- Sidebar (Simplified for brevity, same as index) --}}
        ...
        <main class="app-main">
            <div class="app-content-header">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-sm-6"><h3 class="mb-0">Add New Program</h3></div>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="billing_model" class="form-select @error('billing_model') is-invalid @enderror" required>
                                            ...
                                            <option value="daily" {{ old('billing_model') == 'daily' ? 'selected' : '' }}>Daily</option>
                                            <option value="hourly" {{ old('billing_model') == 'hourly' ? 'selected' : '' }}>Hourly</option>
                                        </select>
                                        @error('billing_model')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Min Age (Months)</label>
                                        <input type="number" name="min_age_months" class="form-control @error('min_age_months') is-invalid @enderror" value="{{ old('min_age_months') }}">
                                        @error('min_age_months')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Max Age (Months)</label>
                                        <input type="number" name="max_age_months" class="form-control @error('max_age_months') is-invalid @enderror" value="{{ old('max_age_months') }}">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <input type="number" step="0.01" name="monthly_fee" class="form-control @error('monthly_fee') is-invalid @enderror" value="{{ old('monthly_fee') }}">
                                        @error('monthly_fee')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Daily Rate ($)</label>
                                        <input type="number" step="0.01" name="daily_rate" class="form-control @error('daily_rate') is-invalid @enderror" value="{{ old('daily_rate') }}">
                                        @error('daily_rate')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Hourly Rate ($)</label>
                                        <input type="number" step="0.01" name="hourly_rate" class="form-control @error('hourly_rate') is-invalid @enderror" value="{{ old('hourly_rate') }}">
                                        @error('hourly_rate')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Start Time</label>
                                        <input type="time" name="day_start_time" class="form-control @error('day_start_time') is-invalid @enderror" value="{{ old('day_start_time') }}">
                                        @error('day_start_time')<div class="invalid-feedback">{{ $message }}</div>@enderror
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">End Time</label>
                                        <input type="time" name="day_end_time" class="form-control @error('day_end_time') is-invalid @enderror" value="{{ old('day_end_time') }}">
